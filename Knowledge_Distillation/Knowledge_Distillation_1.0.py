...
class FC(nn.Module):
    def __init__(self,in_channels=1,num_classes=10):
        super(FC, self).__init__()# 直接调用父类的构造函数
        self.relu = nn.ReLU()
...
for epoch in range(epochs):
    for data,targets in tqdm(train_loder):
        ...
        # 学生模型预测
        student_preds,student_feat = student_model_scratch(data)
        S = fc(student_feat)

        loss3_CE = hard_loss(student_preds,targets)
        loss6_CE = hard_loss(S, targets)
        ...
        loss4_KL = soft_loss(
            F.softmax(S / temp, dim=1),
            F.softmax(teacher_preds / temp, dim=1)
        )

        # 将 hard_loss 和 soft_loss 加权求和
        loss1 = alpha * loss3_CE + (1-alpha) * loss1_KL
        loss2 = alpha * loss6_CE + (1-alpha) * loss4_KL
